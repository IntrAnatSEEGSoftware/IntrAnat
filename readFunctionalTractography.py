...
def readFunctionalTractography(functional_result_file):
    """
        Read a functional tractography CSV file
    """
                    ...
                    if row[0] == 'Patient' or row[0] == 'Atlas':
                        ...
                        if lastParam is not None:
                            if row[0].strip() not in list(full_dictionnary[lastParam].keys()):
                                ...
                                print("WARNING: unexpected lastParam ", repr(lastParam))
                            # if row[0].strip() in full_dictionnary[lastParam].keys():
                            for i_parcels_bis in range(len(row_parcel_names)):
                                full_dictionnary[lastParam][row[0].strip()].update(
                                    {row_parcel_names[i_parcels_bis]: row[i_parcels_bis + 1]})
